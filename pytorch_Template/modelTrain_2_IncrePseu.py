...
class SSL():
    ...
    def main(self):
        self.pre_train(pre_epoch=200)
        train_epoch = 100
        for i in range(train_epoch):
            logging.info(f"第{i}次循环")
            self.formal_train(epoch=i, num=train_epoch)
        logging.info("训练model_last   ###################################")
        self.model_last = self.train(self.hybrid_data1, self.hybrid_label1, self.F1, TOTAL_EPOCHS=100)


    def train(self, train_X, train_Y, model, TOTAL_EPOCHS=50, BATCH_SIZE=100, LEARNING_RATE=0.001, change_learning_rate_epochs=100):
    ...
    def pre_train(self, pre_epoch):
        ...
        self.F1 = self.train(hybrid_data1, hybrid_label1, self.F1)
        ...
        self.F2 = self.train(hybrid_data2, hybrid_label2, self.F2)
        ...
        self.F3 = self.train(hybrid_data3, hybrid_label3, self.F3)


    def formal_train(self, epoch, num):
        """分割未标签数据"""
        # self.split_unlabelset(epoch+1)

        logging.info("正式训练F1、F2、F3   ##################################")
        pseudo_label_data1, pseudo_label1 = self.generate_pseudo_label(self.F2, self.F3, self.unlabel_data, percentage=(epoch+1)/num)
        ...
        self.F1 = self.train(self.hybrid_data1, self.hybrid_label1, self.F1)

        pseudo_label_data2, pseudo_label2 = self.generate_pseudo_label(self.F1, self.F3, self.unlabel_data, percentage=(epoch+1)/num)
        ...
        self.F2 = self.train(self.hybrid_data2, self.hybrid_label2, self.F2)

        pseudo_label_data3, pseudo_label3 = self.generate_pseudo_label(self.F1, self.F2, self.unlabel_data, percentage=(epoch+1)/num)
        ...
        self.F3 = self.train(self.hybrid_data3, self.hybrid_label3, self.F3)
    ...
    def generate_pseudo_label(self, model1, model2, unlabel_data, percentage):
        ...
        pseudo_label_data = self.unlabel_data[diff.sort()[1][:int(len(diff) * percentage)]]
